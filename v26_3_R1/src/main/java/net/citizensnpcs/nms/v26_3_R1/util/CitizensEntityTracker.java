...
public class CitizensEntityTracker extends ChunkMap.TrackedEntity {
    ...
    public CitizensEntityTracker(ChunkMap map, Entity entity, int i, UpdateInterval j, boolean flag) {
    ...
    private static UpdateInterval getUpdateInterval(TrackedEntity entry) {
        try {
            return (UpdateInterval) UPDATE_INTERVAL.invoke(TRACKER_ENTRY.invoke(entry));
        } catch (Throwable e) {
            e.printStackTrace();
        }
        return null;
