 ...
 * underlying {@link Team} and {@link net.megavex.scoreboardlibrary.api.team.ScoreboardTeam} objects. Entries can be
 * either player display names (usernames) or entity UUIDs represented as strings.
 */
public interface AbstractTeam {

    /**
     * Adds an entry to the team. For players, this should be their display name (username). For entities, this should
     * be their UUID represented as a string.
     *
     * @param entry
     *            Entry to add.
     */
    ...
    /**
     * Gets the underlying delegate object. This may either be a {@link Team} or
     * {@link net.megavex.scoreboardlibrary.api.team.ScoreboardTeam}
     *
    ...
    /**
     * Checks if the team has an entry. For players, this should be their display name (username). For entities, this
     * should be their UUID represented as a string.
     *
     * @param entry
     *            Entry to check.
    ...
    /**
     * Removes an entry from the team. For players, this should be their display name (username). For entities, this
     * should be their UUID represented as a string.
     *
     * @param entry
     *            Entry to remove.
    ...
    enum CollisionRule implements TeamOption<net.megavex.scoreboardlibrary.api.team.enums.CollisionRule> {
        ...
        PUSH_OTHER_TEAMS(net.megavex.scoreboardlibrary.api.team.enums.CollisionRule.PUSH_OTHER_TEAMS,
                Team.OptionStatus.FOR_OTHER_TEAMS),
        PUSH_OWN_TEAM(net.megavex.scoreboardlibrary.api.team.enums.CollisionRule.PUSH_OWN_TEAM,
        ...
        CollisionRule(net.megavex.scoreboardlibrary.api.team.enums.CollisionRule packetBasedValue,
                Team.OptionStatus bukkitValue) {
    ...
    interface TeamOption<P> {
        ...
        static <E extends TeamOption<?>> E fromBukkit(E[] values, Team.OptionStatus bukkitValue) {
            for (E e : values) {
                if (e.getBukkitValue() == bukkitValue)
                    return e;
            }
            return values[0];
        }
         ...
         *            Array of enum constants.
         * @param foliaValue
         *            Packet-based (folia) value to convert.
         * @return The corresponding enum constant.
         * @param <E>
         *            Enum type.
         * @param <P>
        ...
        static <E extends TeamOption<P>, P> E fromFolia(E[] values, P foliaValue) {
            for (E e : values) {
                if (e.getFoliaValue().equals(foliaValue))
                    return e;
            }
            return values[0];
        }
