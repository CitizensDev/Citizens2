...
public class EventListen implements Listener {
    ...
    private void respawnAllFromCoord(ChunkCoord coord, Event event) {
        ...
        for (int i = 0; i < ids.size(); i++) {
            ...
            // Despawned NPC respawn blocking
            if (npc.getOrAddTrait(ForceDespawnTrait.class).isDespawned()) {
                Messaging.debug("NPC", npc.getId(), " spawn blocked due to force despawn trait");
                continue;
            }
